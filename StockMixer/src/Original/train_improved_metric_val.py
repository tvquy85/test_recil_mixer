...
def main() -> None:
    ...
    for epoch in range(epochs):
        ...
        # update best performances based on the selected criterion
        if args.select_by == "loss":
            # minimise validation loss (same behaviour as train.py)
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                best_val_perf = val_perf
                best_test_perf = test_perf
        else:
            # maximise IC, RIC or IC+RIC depending on select_by
            if args.select_by == "ic":
                cur_score = val_perf.get("IC", float("-inf"))
            elif args.select_by == "ric":
                cur_score = val_perf.get("RIC", float("-inf"))
            elif args.select_by == "ic+ric":
                # handle missing values gracefully
                ic = val_perf.get("IC", float("-inf"))
                ric = val_perf.get("RIC", float("-inf"))
                cur_score = ic + ric
            else:
                cur_score = float("-inf")
            # update best score if current is higher and finite
