...
# === Market State Gating and Market Context Aware GatedMLP ===

class MarketStateGate(nn.Module):
    """
    Project a vector of market state metrics to the feature dimension and produce
    a gating vector in `[0, 1]` using a small feed‑forward network.  This
    module allows the model to incorporate handcrafted global market context
    features (e.g., mean return, slope, volatility, dispersion, PCA ratio)
    computed externally from closing prices.

    Parameters
    ----------
    metrics_dim : int
        The number of market state features provided for each window.
    feature_dim : int
        The dimensionality of the per‑stock embeddings to be gated.
    """
    def __init__(self, metrics_dim: int, feature_dim: int):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(metrics_dim, feature_dim),
            nn.Hardswish(),
            nn.Linear(feature_dim, feature_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self, metrics: torch.Tensor) -> torch.Tensor:
        ...
        metrics : torch.Tensor
            A tensor of shape ``(metrics_dim,)`` or ``(batch_size, metrics_dim)``
            containing the market state features for the current window.

        Returns
        -------
        torch.Tensor
            A gating vector of shape ``(feature_dim,)`` or ``(batch_size, feature_dim)``
            with values in `[0, 1]`.
        """
        return self.fc(metrics)


class MarketContextAwareGatedMLP(nn.Module):
    """
    Extension of ``GatedMLPStockModel`` that integrates handcrafted market
    state features into the gating mechanism.  The model accepts, in addition
    to the per‑stock time series, a vector of market features that describe
    global conditions such as mean return, momentum, volatility, dispersion
    and PCA ratio over the lookback window.  These metrics are projected to
    the hidden dimension via a ``MarketStateGate`` and combined multiplicatively
    with the standard market context gate.  This allows the model to adjust
    the emphasis on individual embedding dimensions based on both internal
    stock statistics and external market indicators.

    The remainder of the architecture mirrors ``GatedMLPStockModel``: indicator
    mixing via an MLP, temporal mixing via a gMLP block, optional cross‑stock
    attention, and a final linear head.
    """
    def __init__(self, stocks: int, time_steps: int, channels: int,
                 hidden_dim: int = 32, market_metrics_dim: int = 5,
                 use_cross_attn: bool = True, num_heads: int = 4):
        super().__init__()
        self.stocks = stocks
        self.time_steps = time_steps
        self.channels = channels
        self.hidden_dim = hidden_dim
        self.market_metrics_dim = market_metrics_dim
        # indicator mixing
        self.indicator_mlp = IndicatorMLP(channels, hidden_dim, hidden_dim)
        # gMLP for temporal mixing
        self.time_mixer = GatedMLPBlock(input_dim=hidden_dim,
                                        hidden_dim=hidden_dim,
                                        seq_len=time_steps)
        # gating based on per‑stock embeddings (global context)
        self.market_gate = MarketContextGating(feature_dim=hidden_dim)
        # gating based on external market state features
        self.market_state_gate = MarketStateGate(metrics_dim=market_metrics_dim,
    ...
    def forward(self, inputs: torch.Tensor, market_state: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through the ``MarketContextAwareGatedMLP``.
        ...
        market_state : torch.Tensor
            Tensor of shape ``(market_metrics_dim,)`` or ``(batch_size, market_metrics_dim)``
            containing the market state metrics for the current window.  In the
            typical use case, the metrics vector is computed over the same
            lookback window as ``inputs``.
        ...
        torch.Tensor
            ...
            return ratio for each stock.
        """
        # 1. mix indicators at each time step
        x = self.indicator_mlp(inputs)  # (stocks, time_steps, hidden_dim)
        # 2. time mixing via gMLP
        x = self.time_mixer(x)  # (stocks, time_steps, hidden_dim)
        # 3. average pool over time
        x = x.mean(dim=1)  # (stocks, hidden_dim)
        # 4. compute global market gate from embeddings
        x = self.market_gate(x)  # (stocks, hidden_dim)
        # 5. compute external market state gate (broadcast across stocks)
        # ensure market_state has shape (batch_size, metrics_dim) or (metrics_dim,)
        if market_state.dim() == 1:
            gating_state = self.market_state_gate(market_state.unsqueeze(0))  # (1, hidden_dim)
            gating_state = gating_state.squeeze(0)  # (hidden_dim,)
        else:
            gating_state = self.market_state_gate(market_state)  # (batch, hidden_dim)
        # apply gating: multiply each stock embedding by the metrics gate
        x = x * gating_state  # broadcasting along stocks
        # 6. optional cross-stock attention
        if self.use_cross_attn:
            x = self.cross_attn(x)
        # 7. regression head
        out = self.head(x)
