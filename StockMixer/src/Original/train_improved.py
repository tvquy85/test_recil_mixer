...
from improved_models import get_loss, GatedMLPStockModel, CrossAttentionStockModel, MarketContextAwareGatedMLP
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str,
                        choices=["stock_mixer", "gated_mlp", "cross_attn", "market_gated"],
                        default="gated_mlp",
                        help=("Model architecture to use: stock_mixer, gated_mlp, cross_attn or "
                              "market_gated (MarketContextAwareGatedMLP)") )
...
def main() -> None:
    ...
    if args.market == "SP500":
        ...
        sp500_path = os.path.join(BASE_DIR, "dataset", "SP500", "SP500.npy")
    ...
    market_name_list = ["NASDAQ", "SP500", "crypto"]
    stock_num_list = [1026, 474, 117]
    valid_index_list = [756, 1006, 620]
    test_index_list = [1008, 1259, 827]
    market_values = [20, 8, 20]
    depth_values = [3, 5, 5]
    ...
    # Ánh xạ sang các giá trị
    if market_name in market_name_list:
        idx = market_name_list.index(market_name)

        stock_num = stock_num_list[idx]
        valid_index = valid_index_list[idx]
        test_index = test_index_list[idx]
        market_dim = market_values[idx]
        scale_factor = depth_values[idx]

        print(f"Market: {market_name}")
        print(f"  stock_num = {stock_num}")
        print(f"  valid_index = {valid_index}")
        print(f"  test_index = {test_index}")
        print(f"  market_dim = {market_dim}")
        print(f"  scale_factor = {scale_factor}")
    else:
        raise ValueError(f"Unknown market_name: {market_name}")
    # compute market state metrics (5 features) for each possible window
    # using the preprocess function.  The window length matches lookback_length.
    # market_metrics has shape (trade_dates - lookback_length, 5), where each row
    # corresponds to the metrics computed over a window of length lookback_length.
    #market_metrics = market_state_from_closes(eod_data, close_col=fea_num - 1, window=lookback_length)
    import traceback
    metrics_filename = os.path.join(dataset_path, f"{args.market}market_metrics_{lookback_length}.pkl")
    if os.path.exists(metrics_filename):
        with open(metrics_filename, "rb") as f:
            market_metrics = pickle.load(f)
    else:
        market_metrics = market_state_from_closes(eod_data, close_col=fea_num - 1, window=lookback_length)
        # save the computed metrics for future runs
        try:
            with open(metrics_filename, "wb") as f:
                pickle.dump(market_metrics, f)
        except Exception as e:
            # in ra loại lỗi và thông tin chi tiết
            print(f"[Warning] Could not save market_metrics to {metrics_filename}")
            print(f"Exception: {e}")
            traceback.print_exc()
            # nếu muốn vẫn tiếp tục thì giữ lại pass
            pass
    ...
    if args.model == "stock_mixer":
        ...
        # market_gated: use the MarketContextAwareGatedMLP
        model = MarketContextAwareGatedMLP(stocks=stock_num,
                                           time_steps=lookback_length,
                                           channels=fea_num,
                                           hidden_dim=32,
                                           market_metrics_dim=market_metrics.shape[1],
                                           use_cross_attn=True,
                                           num_heads=4)
    ...
    def get_batch(offset: int):
        ...
        # extract corresponding market metrics for this window; metrics are
        # aligned such that metrics[k] corresponds to the window starting at k
        if offset < market_metrics.shape[0]:
            metrics_vec = market_metrics[offset]  # (metrics_dim,)
        else:
            # if offset exceeds precomputed metrics length, use last available
            metrics_vec = market_metrics[-1]
        return (
    ...
    def validate(start_index: int, end_index: int):
        ...
        with torch.no_grad():
            ...
            for cur_offset in range(start_index - lookback_length - steps + 1,
                ...
                # unpack data and convert to torch tensors
                data_np, mask_np, price_np, gt_np, metrics_np = get_batch(cur_offset)
                data_batch = torch.tensor(data_np, dtype=torch.float32).to(device)
                mask_batch = torch.tensor(mask_np, dtype=torch.float32).to(device)
                price_batch = torch.tensor(price_np, dtype=torch.float32).to(device)
                gt_batch = torch.tensor(gt_np, dtype=torch.float32).to(device)
                metrics_batch = torch.tensor(metrics_np, dtype=torch.float32).to(device)
                # forward pass depends on model type
                if args.model == "market_gated":
                    preds = model(data_batch, metrics_batch)
                else:
    ...
    for epoch in range(epochs):
        ...
        for j in range(valid_index - lookback_length - steps + 1):
            # unpack data and convert to tensors
            data_np, mask_np, price_np, gt_np, metrics_np = get_batch(batch_offsets[j])
            data_batch = torch.tensor(data_np, dtype=torch.float32).to(device)
            mask_batch = torch.tensor(mask_np, dtype=torch.float32).to(device)
            price_batch = torch.tensor(price_np, dtype=torch.float32).to(device)
            gt_batch = torch.tensor(gt_np, dtype=torch.float32).to(device)
